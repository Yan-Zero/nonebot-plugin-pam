...
function HomePage() {
  ...
  useEffect(() => {
    const fetchPlugins = async () => {
      try {
        const pluginsResponse = await axios.post('/pam/api/plugins', {}, {
          headers: {
        ...
        await Promise.all(
          Object.keys(plugins).map(async (plugin) => {
            const rulesResponse = await axios.post('/pam/api/fetch', { plugin },
              {
                headers: {
                  Authorization: getAuthToken(),
                },
      ...
      <div style={
        ...
        <div style={{
          ...
          {loading ? (
            ...
            <MultiLevelList data={data} onSelect={handleSelect} />
          )}
        </div>
        <div style={{
          padding: "30px",
